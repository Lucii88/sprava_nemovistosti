...
| Poznámka s volitelným termínem (zobrazí se na nástěnce) | ne | pokrývá její vlastní příklad („do konce smlouvy nová požární zpráva"); jedno datumové pole navíc | **V1 light** — potvrdit na brainstormingu |
...
| Roční vyúčtování záloh na služby (odečty měřidel, lhůta 4 měsíce) | ne | velký modul; nevíme, zda vyúčtování dělá ona, nebo účetní, či zda jede paušály | **❓ Brainstorming** → případně etapa 2 |
| Inflační doložka / automatická indexace nájmu | ne | nevíme, zda ji smlouvy obsahují; datový model změny cen v čase pokrývá i bez automatiky | **❓ Brainstorming** → případně etapa 2 |
| Evidence kaucí/jistot | ne | jedno pole + poznámka; ale nevíme, zda kauce vybírají | **❓ Brainstorming** |
...
| Napojení na účetní software (Pohoda apod.) | ne | účetnictví dělá externí firma vlastním systémem; Excel export stačí | **Zamítnuto pro V1** — na brainstormingu zjistit, co účetní používá |
...
Evidence (entity, nemovitosti, nájemci, smlouvy s položkami) · generátor platebních kalendářů (jednotlivě i hromadně, s validacemi a archivem PDF) · historie nájemce + poznámky (s volitelným termínem) · minimální nástěnka · filtry + export do Excelu · role a účty · audit log · administrace (entity, sazby DPH, typy nemovitostí, uživatelé) · jednorázový import z Excelu. **Nic víc.**

## F. Otázky na brainstorming, které mohou rozsah změnit

1. Roční vyúčtování záloh — dělá ho ona, účetní, nebo jedou paušály bez vyúčtování?
2. Obsahují smlouvy inflační doložky? (→ indexace jako nadstavba)
3. Vybírají kauce/jistoty? (→ jedno pole navíc)
4. Poznámka s termínem na nástěnce — chce to tak?
5. Vzor platebního kalendáře a Excel (čekáme) — ověřit pole šablony 1:1.
6. Co používá externí účetní — kvůli budoucím exportům.
7. Přístup jen z firemní sítě, nebo i přes VPN? Skrýt soukromou entitu ostatním uživatelům?
8. Číslování kalendářů — existuje řada, na kterou navázat?
...
- `Smlouva` — entita + nájemce + nemovitost(i); právní režim (nájem / prostor k podnikání / pacht), doba určitá/neurčitá, období, **pravidlo splatnosti** („X. den předchozího měsíce"), **režim DPH** (osvobozeno / volba / povinně).
- `Položka smlouvy` — typ (nájem / energie-služby / voda / …), částka, sazba DPH (odkaz na číselník s časovou platností), režim (plnění s DPH / přeúčtování mimo DPH), **platnost od–do** — změna ceny = nový řádek platnosti, ne přepis (⇒ historie cen zdarma).
...
- `Uživatel` + role; `AuditLog` — append-only.
...
3. **Etapa 2 — jen na vyžádání po zkušenosti s V1:** modul Platby (KB výpisy, párování, podklady KH), roční vyúčtování služeb, indexace, QR platba, kontrola insolvence.

...
Menu (ploché, bez žargonu, **řazené podle četnosti použití**): Nástěnka · Nájemci · Platební kalendáře · Smlouvy · Nemovitosti · **Nastavení** (zde se zakládají firmy). **V uživatelském rozhraní se „entita" vždy označuje jako „firma"** — jazyk klientky (viz `persona-lenka-vcelakova.md`); „entita" zůstává technickým pojmem dokumentace a datového modelu. Stejně tak žádné paragrafy v UI — česky („s DPH 21 %", „bez DPH"); § patří jen na PDF doklad.

1. **Nástěnka (vítací stránka)** — odpovídá na jedinou otázku: *„musím dnes něco udělat?"* Stavěná na **skenování, ne čtení**: (a) osobní pozdrav křestním jménem — **deterministicky** z pole „oslovení" (5. pád) v profilu uživatele, předvyplněného pravidly českého skloňování a ručně upravitelného; (b) semafor v jedné řádce; (c) **hero karta** s jedinou nejnaléhavější věcí a tlačítkem akce; (d) **Co mě čeká** — přepínač **Seznam / Kalendář** nad stejnými daty: seznam řazený podle data (datum vlevo v rámečku, celé jméno nájemce, akce vpravo) a **měsíční kalendář** s termíny přímo ve dnech (barevně: červená spěchá, jantarová termín, mátová splatnosti dle vystavených kalendářů), s listováním po měsících a tlačítkem Dnes; (e) Portfolio v kostce (jen počty, žádné grafy); (f) Pro informaci (volné prostory).
   **Jména nájemců se všude uvádějí celá a prokliknou přímo do detailu daného nájemce** — nikdy jen do obecného seznamu.
